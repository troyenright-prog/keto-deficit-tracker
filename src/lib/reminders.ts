...
export const DEFAULT_REMINDERS: ReminderSettings = {
  ...
  weighIn: { enabled: false, time: '07:00', weekday: 2 },
  electrolytes: { enabled: false, time: '14:00' },
  shopping: { enabled: false, time: '18:00', weekday: 6 },
};
...
function normalizeWeeklyRule(value: unknown, fallback: WeeklyReminderRule): WeeklyReminderRule {
  ...
  return {
    ...
    weekday: normalizeWeekday(record.weekday, fallback.weekday),
  };
}
...
export function buildReminderNotifications(settings: ReminderSettings): LocalNotificationSchema[] {
  ...
  const addWeekly = (
    ...
    notifications.push({
      ...
      autoCancel: true,
      schedule: {
        on: { ...timeParts(rule.time), weekday: rule.weekday as Weekday },
        repeats: true,
        allowWhileIdle: true,
      },
      extra: { reminder: key },
...
export async function scheduleReminderNotifications(settings: ReminderSettings): Promise<ReminderScheduleResult> {
  ...
  try {
    ...
    await LocalNotifications.cancel({
      notifications: Object.values(REMINDER_NOTIFICATION_IDS).map((id) => ({ id })),
    });
