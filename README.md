...
The Scan screen supports packaged-food lookup by barcode. Native Android builds use the free on-device ML Kit scanner with automatic zoom and haptic confirmation; web builds retain the ZXing camera scanner. Every browser can still use barcode number entry.

Barcode lookup uses a Cloudflare Pages Function at `/api/lookup-barcode`, which queries Open Food Facts first and can fall back to USDA FoodData Central when configured, including when Open Food Facts is temporarily rate-limited. Successful responses are cached at the edge for seven days and shared automatically across devices. UPC/EAN/GTIN container variants are matched through a common GTIN-14 comparison key. Native builds can also point at the deployed function with `VITE_BARCODE_LOOKUP_URL` so scans use the broader server-side lookup instead of only the direct Open Food Facts fallback. Results are normalized for review before logging. Logged entries and saved foods are copied as snapshots, so future changes in the external database do not alter historical logs.
